...
const StudentsSlotsDetails = ({ onClose, booking_id, allBooking }) => {
  const handleClose = () => {
    onClose();
  };

  const filteredBooking = allBooking?.find(
  ...
  // Group slots by date
  const groupedSlots = booking?.booking?.slots?.reduce((groups, slot) => {
            ...
            <div id="tuturn-model-body">
              <div className="modal-header">
                <div className="tu-popimghead">
                  <img
                    src={booking?.booking?.tutor?.image || avatarImage}
                    alt="Tutor"
                  />
                  <h5>
                    {booking?.booking?.tutor?.first_name}&nbsp;
                    {booking?.booking?.tutor?.last_name} booking details
                  </h5>
                </div>
                <a
                  className="tu-close"
                ...
                <div className=" mCustomScrollbar _mCS_2">
                  ...
                  >
                    <div
                      ...
                      dir="ltr"
                    >
                      {booking?.booking?.slots?.length === 0 ? (
...
export default StudentsSlotsDetails;
